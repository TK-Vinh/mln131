...
export function BlogReader({ blog }: BlogReaderProps) {
  ...
  const handleScroll = useCallback(() => {
    if (articleRef.current) {
      ...
      const totalScrollDistanceForArticle = articleContentHeight - clientHeight;

      let currentProgress = 0;
      if (totalScrollDistanceForArticle > 0) {
        currentProgress =
          (scrolledWithinArticle / totalScrollDistanceForArticle) * 100;
      } else if (articleContentHeight <= clientHeight) {
        currentProgress =
          articleRect.top <= 0 && articleRect.bottom >= clientHeight ? 100 : 0;
      }
      ...
      <article ref={articleRef} className="max-w-6xl mx-auto px-4 py-8">
        <header className="mb-8 text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            <Badge variant={languageBadge.variant}>{languageBadge.label}</Badge>
            ...
            <Card className="w-full mb-4">
              ...
              <CardContent>
                <div className="flex flex-col gap-4">
                  <Select
                    value={audioLanguage}
                    onValueChange={(value: Language) => setAudioLanguage(value)}
                  >
                ...
                p: ({ node, ...props }) => {
                  const isPseudoHeading =
                    ...
                    (props.children.includes("Khái niệm") ||
                      props.children.includes("Kết luận") ||
                      /^\d+\.\s/.test(props.children));

                  if (isPseudoHeading) {
                    return (
                      <p className="text-lg font-bold mt-8 mb-4" {...props} />
                    );
                  }
                  return (
                    <p
